...
export default function LoginPage() {
  ...
  const handleSocialAuth = async (provider: string) => {
    try {
      ...
      // O redirecionamento será feito automaticamente pelo callback OAuth
    } catch (err: any) {
  ...
  const handleEmailLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      try {
        ...
        // Login bem-sucedido, redirecionar para quiz
        router.push("/quiz-inicial")
      } catch (loginError: any) {
        // Se o erro for "Invalid login credentials", tentar criar conta
        if (loginError.message?.includes("Invalid login credentials") || 
            loginError.message?.includes("Email not confirmed")) {
          try {
            await signUpWithEmail(email, password)
            // Conta criada com sucesso, redirecionar para quiz
            router.push("/quiz-inicial")
      ...
      <div className="w-full max-w-md">
        ...
        <div className="bg-white rounded-3xl shadow-2xl p-8 sm:p-10 border border-gray-100">
          ...
          <form onSubmit={handleEmailLogin} className="space-y-4">
            ...
            >
              {loading ? "Processando..." : "Entrar com e-mail"}
            </button>
          </form>
          ...
          <button
            ...
            className="w-full mt-4 bg-yellow-500 text-gray-900 px-6 py-3.5 rounded-xl font-semibold hover:bg-yellow-400 transition-all hover:shadow-lg hover:scale-[1.02] active:scale-[0.98]"
          >
            Entrar sem login (modo desenvolvedor)
          </button>
          ...
          <p className="text-xs text-center text-gray-500 mt-6 leading-relaxed">
            Ao continuar com Google, Apple ou e-mail, você concorda com os{" "}
            <Link href="/terms" className="text-[#1F75FE] hover:underline">
              Termos de Uso
            </Link>{" "}
            e{" "}
            <Link href="/privacy" className="text-[#1F75FE] hover:underline">
